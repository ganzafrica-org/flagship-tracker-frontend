...
export default function FlagshipSummaryModal({
  ...
  return (
    <div className="fixed inset-0 z-[10] flex items-start justify-center bg-black/65 p-4 md:items-center">
      <Card className="w-full max-w-[80%] rounded-2xl border border-white/10 bg-[#1f2226] text-white shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-white/10 px-4 py-3 md:px-5">
          <div>
            <Chip size="sm" variant="flat" className="h-6 rounded-md bg-[color:var(--accent)]/25 px-2 text-xs text-white">
              Flagship Project
            </Chip>
            <h2 className="mt-2 text-md font-bold leading-tight md:text-lg">{intro.displayTitle}</h2>
        ...
        <div className="grid gap-3 px-4 pb-3 md:grid-cols-3 md:px-5">
          ...
          <SummaryBlock title="Progress & investment">
            ...
            <h4 className="mt-4 text-xs font-semibold text-white/90">Investment sources</h4>
            <div className="mt-1.5 space-y-1.5">
              {flagshipDetailInvestmentSplit.map((row) => (
                <div key={row.name} className="grid grid-cols-[1fr_110px_auto] items-center gap-2 text-xs">
            ...
            <div className="space-y-2">
              {flagshipDetailHighlights.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="rounded-md border border-white/10 px-2.5 py-2 text-xs"
            ...
            <div className="space-y-2">
              {flagshipDetailLocations.slice(0, 3).map((row) => (
                <div key={row.id} className="rounded-md border border-white/20 px-2.5 py-2 leading-snug">
                  ...
                  <p className="text-xs text-white/80">{row.detail}</p>
                </div>
              ))}
            </div>
          </SummaryBlock>
            ...
            <div className="space-y-2">
              {flagshipDetailTeam.slice(0, 4).map((member) => (
                <div key={member.id} className="flex items-center gap-2 rounded-full bg-[#2a2d31] px-2.5 py-1.5">
                  <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-[#e8f4ff] text-xs font-bold text-[#0f172a]">
                    {member.initials}
                  </span>
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-[#262a2f] p-3">
      ...
      <p className="mt-1 text-[38px] font-bold leading-none text-white">
        {value ?? "-"}
        {suffix ? <span className="ml-1 text-xs font-semibold text-white/70">{suffix}</span> : null}
      </p>
    </div>
  );
}
...
function SummaryBlock({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="rounded-xl border border-white/10 bg-[#262a2f] p-3">
      <h3 className="mb-2 text-sm font-bold leading-none tracking-tight text-white/95">{title}</h3>
      {children}
    </div>
  );
}
...
function StatLine({
  ...
  return (
    <div className="grid grid-cols-[auto_1fr_auto_auto] items-center gap-2 py-0.5 text-xs">
